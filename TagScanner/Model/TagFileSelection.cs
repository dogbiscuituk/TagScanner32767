...
namespace TagScanner
{
    ...
    public class TagFileSelection : ITagFile
    {
        #region Lifetime Management

        public TagFileSelection(IEnumerable<ITagFile> files)
        {
            Files = files;
        }
		...
		[Browsable(false)]
        public IEnumerable<ITagFile> Files { get; private set; }
        ...
        private uint? _beatsPerMinute = uint.MaxValue;
        /*[Category("Details")]*/
        [DefaultValue(0)]
        ...
        {
            get { return GetUint(p => p.BeatsPerMinute, ref _beatsPerMinute); }
            set
            {
	            SetValue(p => p.BeatsPerMinute = value);
	            _beatsPerMinute = uint.MaxValue;
            }
        }
        ...
        private uint? _disc = uint.MaxValue;
        [Category("Media")]
        [DefaultValue(0)]
        ...
        public uint? Disc
        {
            get { return GetUint(p => p.Disc, ref _disc); }
            set
            {
	            SetValue(p => p.Disc = value);
				_disc = uint.MaxValue;
            }
        }

        private uint? _discCount = uint.MaxValue;
        [Category("Media")]
        [DefaultValue(0)]
        ...
        {
            get { return GetUint(p => p.DiscCount, ref _discCount); }
            set
            {
	            SetValue(p => p.DiscCount = value);
				_discCount = uint.MaxValue;
            }
        }
        ...
        private bool _isClassicalCalculated;
        [Category("Details")]
        ...
        public bool? IsClassical
        {
            get
            {
                if (!_isClassicalCalculated)
                {
                    _isClassical = GetBool(p => p.IsClassical);
                    _isClassicalCalculated = true;
                }
                return _isClassical;
            }
        }
        ...
        public bool? IsEmpty
        {
            get { return GetBool(p => p.IsEmpty); }
        }

        private string _joinedAlbumArtists;
        [Category("Personnel")]
		[Description("A string containing the band or artist who is credited in the creation of the entire album or collection containing the media described by the selected item(s), or null if no value is present.")]
        ...
        private uint? _track = uint.MaxValue;
        [Category("Media")]
        [DefaultValue(0)]
        ...
        public uint? Track
        {
            get { return GetUint(p => p.Track, ref _track); }
            set
            {
	            SetValue(p => p.Track = value);
				_track = uint.MaxValue;
            }
        }

        private uint? _trackCount = uint.MaxValue;
        [Category("Media")]
        [DefaultValue(0)]
        ...
        {
            get { return GetUint(p => p.TrackCount, ref _trackCount); }
            set
            {
	            SetValue(p => p.TrackCount = value);
				_trackCount = uint.MaxValue;
            }
        }
        ...
        private uint? _year = uint.MaxValue;
        [Category("Details")]
        [DefaultValue(0)]
        ...
        public uint? Year
        {
            get { return GetUint(p => p.Year, ref _year); }
            set
            {
		...
		protected virtual string AggregateStrings(Func<ITagFile, string> getString)
        {
            return AggregateStringsBlank(getString);
        }

        protected string AggregateStringsBlank(Func<ITagFile, string> getString)
        {
            var result = string.Empty;
            var first = true;
            foreach (var value in Files.Select(getString))
                if (first)
                {
                    result = value;
                    first = false;
        ...
        protected string AggregateStringsCount(Func<ITagFile, string> getString)
        {
            var cache = new List<string>();
            var count = 0;
            foreach (var value in Files
                .Select(getString)
                .Where(value => !cache.Contains(value)))
            {
                cache.Add(value);
                count++;
            }
            ...
            return string.Format("({0} values)", count);
        }

        protected virtual uint? AggregateUints(Func<ITagFile, uint?> getUint)
        {
            return AggregateUintsBlank(getUint);
        }

        protected uint? AggregateUintsBlank(Func<ITagFile, uint?> getUint)
        {
            uint? result = null;
            if (Files != null)
            {
                var first = true;
                foreach (var value in Files.Select(getUint))
                    if (first)
                    {
                         result = value;
                         first = false;
        ...
        protected uint? AggregateUintsCount(Func<ITagFile, uint?> getUint)
        {
		...
		private bool? GetBool(Func<ITagFile, bool?> getBool)
        {
            bool? result = null;
            if (Files != null)
            {
                var first = true;
                foreach (var value in Files.Select(getBool))
                {
                    if (first)
                    {
                        result = value;
	    ...
	    private string GetFileOrCommonFolderPath(Func<ITagFile, string> getString, ref string result)
	    {
		    if (result == null)
		    {
			    result = string.Empty;
			    if (Files != null && Files.Any())
			    {
				    ...
				    var path = result;
				    var paths = Files.Select(getString).ToList();
	    ...
	    private int? GetInt(Func<ITagFile, int?> getInt, ref int? result)
        {
            if (result == int.MaxValue)
            {
                ...
                {
                    var first = true;
                    foreach (var value in Files.Select(getInt))
                    {
                        if (first)
                        {
                            result = value;
        ...
        private long? GetLong(Func<ITagFile, long?> getLong, ref long? result, bool sum)
        {
            if (result == long.MaxValue)
            {
                ...
                {
                    var first = true;
                    foreach (var value in Files.Select(getLong))
                        if (first)
                        {
                            result = value;
                            first = false;
        ...
        private Picture[] GetPictures(Func<ITagFile, Picture[]> getPictures, ref Picture[] result)
        {
            if (result == null)
            {
                var pictureList = new List<Picture>();
                if (Files != null)
                    foreach (var pictures in Files
                        .Select(getPictures)
                        .Where(p => p != null))
                    {
        ...
        private string GetString(Func<ITagFile, string> getString, ref string result)
        {
            return result ?? (result = AggregateStrings(getString));
        }

        private string[] GetStringArray(Func<ITagFile, string[]> getStringArray, ref string[] result)
        {
            if (result == null)
            {
                ...
                if (Files != null)
                    foreach (var t in
                        from s in Files.Select(getStringArray)
                        where s != null
                        from t in s.Where(t => !values.Contains(t))
                        select t)
                        values.Add(t);
        ...
        private TagLib.TagTypes GetTagTypes(Func<ITagFile, TagLib.TagTypes> getTagTypes, ref TagLib.TagTypes result)
        {
        ...
        private TimeSpan? GetTimeSpan(Func<ITagFile, TimeSpan?> getTimeSpan, ref TimeSpan? result)
        {
        ...
        private uint? GetUint(Func<ITagFile, uint?> getUint, ref uint? result)
        {
            if (result == uint.MaxValue)
                result = AggregateUints(getUint);
            return result;
        }
		...
		private void SetValue(Action<ITagFile> setValue)
        {
