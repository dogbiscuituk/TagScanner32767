...
{
	public class ColumnChooserController
	{
		#region Public Interface

		#region Lifetime Management

		public ColumnChooserController(Control host, bool checkBoxes)
		{
			View = new ColumnChooser
			{
				listView2 = { CheckBoxes = checkBoxes },
				lblCheckBoxes = { Visible = checkBoxes }
			};
		...
		public void Init(IEnumerable<string> availableColumns, IEnumerable<Order> selectedOrders)
		{
			ListView1.Items.Clear();
			foreach (var propertyTag in
				from column in availableColumns
				let order = selectedOrders.FirstOrDefault(p => p.PropertyName == column)
				where order == null
				select column)
				ListView1.Items.Add(propertyTag.ToString());
